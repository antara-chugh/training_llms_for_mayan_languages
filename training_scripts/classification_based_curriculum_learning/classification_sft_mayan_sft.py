...
stage2_all_mayan.py
===================
Stage 2 — Experiment 1: Translation SFT on ALL Mayan↔English pairs.
Starts from the Stage 1 LangID adapter (langid_sft/best).

Train: train splits for all Mayan languages + Q'anjob'al train split
Eval:  test splits for all Mayan languages + Q'anjob'al test split

Usage:
    python stage2_all_mayan.py
    python stage2_all_mayan.py --adapter langid_sft/best

Outputs:
    stage2_all_mayan/best/                  <- final LoRA adapter
    stage2_all_mayan/translation_eval.json  <- per-language BLEU + chrF
    stage2_all_mayan/plots/                 <- training curves
"""
...
OUTPUT_DIR       = "stage_classification_2_all_mayan"
...
def load_data(tokenizer) -> tuple:
    ...
    for lang_code, lang_name in LANGUAGE_META.items():
        train_path = os.path.join(SPLITS_DIR, lang_code, "train")
        if os.path.exists(train_path):
            ...
            for e, m in zip(ds[eng_col], ds[may_col]):
                ...
                if eng and may:
                    all_train.append({
                        "text": build_translation_prompt_train(tokenizer, eng, may, lang_name),
                    })
                    count += 1
            print(f"  [{lang_code}] {lang_name} train: {count}")
        else:
            print(f"  [{lang_code}] train not found at {train_path}, skipping")
    ...
    # SFT eval subset for loss tracking during training
    sft_eval_size = min(500, len(all_train) // 10)
...
def plot_results(output_dir, trans_results, state_path):
    ...
    if train_losses or eval_losses:
        ...
        fig.suptitle("Stage 2 (All Mayan) — Training Curves", fontsize=13, fontweight="bold")
        if train_losses:
    ...
    if lang_results:
        ...
        ax.set_title("Per-Language Translation Scores (All Mayan)", fontweight="bold")
...
def main():
    ...
    print(f"Experiment      : All Mayan translation pairs")
    ...
    print("\nTraining Stage 2 (All Mayan Translation SFT)...")
    trainer.train()
    ...
    # ── Translation eval ───────────────────────────────────────────────────────
    print("\nRunning per-language translation eval...")
    ...
    print(f"\n{'='*55}")
    print(f"Stage 2 (All Mayan) complete.")
